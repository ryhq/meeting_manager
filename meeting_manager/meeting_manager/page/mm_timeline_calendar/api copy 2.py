...
def update_booking(booking_id, start_datetime=None, end_datetime=None, new_host=None, department=None):
	...
	- New host must be an active member of the selected department
	...
	Args:
		...
		department (str, optional): Department slug (required for reassignment)
	...
	try:
		...
		# Check permissions
		if not booking.has_permission("write"):
			return {
				"success": False,
		...
		# Handle time change (rescheduling)
		if start_datetime and end_datetime:
			...
			# Parse ISO datetime and convert to MySQL-compatible format
			# Handle both formats: '2026-01-02T04:30:00.000Z' and '2026-01-02T04:30:00Z'
			...
			booking.start_datetime = start_dt.strftime('%Y-%m-%d %H:%M:%S')
			booking.end_datetime = end_dt.strftime('%Y-%m-%d %H:%M:%S')

			# Recalculate duration
			duration = int((end_dt - start_dt).total_seconds() / 60)
			booking.duration = duration

			changes.append(f"Rescheduled from {old_start} to {booking.start_datetime}")

		# Handle host change (reassignment)
		if new_host and department:
			# Verify new host is an active member of the department
			...
			if not is_member:
				return {
					"success": False,
					"message": f"User {new_host} is not an active member of the selected department"
				}

			# Get current assigned users
			current_hosts = [au.user for au in booking.assigned_users]

			if new_host not in current_hosts:
				# Find the primary host to replace
				primary_host = None
				...
				if primary_host:
					old_host = primary_host.user
					old_host_name = frappe.get_value("User", old_host, "full_name") or old_host
					new_host_name = frappe.get_value("User", new_host, "full_name") or new_host

					primary_host.user = new_host
					changes.append(f"Reassigned from {old_host_name} to {new_host_name}")
				else:
					# No primary host found, add as new assigned user
					new_host_name = frappe.get_value("User", new_host, "full_name") or new_host
					booking.append("assigned_users", {
						"user": new_host,
						"is_primary_host": 1
					})
					changes.append(f"Assigned to {new_host_name}")
		elif new_host and not department:
			return {
				"success": False,
				"message": "Department must be selected to reassign meetings"
			}
